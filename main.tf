...
resource "azurerm_subnet" "subnet_count" {
  ...
  private_endpoint_network_policies             = lookup(var.subnet_private_endpoint_network_policies, var.subnet_names[count.index], false)
...
resource "azurerm_subnet" "subnet_for_each" {
  ...
  resource_group_name                           = var.resource_group_name
  virtual_network_name                          = azurerm_virtual_network.vnet.name
  private_endpoint_network_policies             = lookup(var.subnet_private_endpoint_network_policies, each.value, false)
